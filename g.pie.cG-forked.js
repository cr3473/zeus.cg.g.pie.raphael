...
(function () {
    ...
    function Piechart(paper, cx, cy, r, values, opts) {
		...
		// dictionary to hold id:slice
		chart.slices = {};

		if (valuesLen == 0) {
			var p = paper.circle(cx, cy, r)
				.attr(
				{ 
					fill: "#fff", 
					stroke: "#000", 
            ...
            var s = paper.circle(cx, cy, r).attr(chartinst.shim);
            ...
            values[0].order = 0;
            values[0].valueOf = function () { return this.value; };
            ...
            for (var i = 0; i < valuesLen; i++) {
                ...
                // values[i] = { value: values[i].value, order: i, valueOf: function () { return this.value; } };
                values[i].order = i;
                values[i].valueOf = function () { return this.value; };
            }
            ...
            valuesLen = Math.min(cut + 1, values.length);
            others && values.splice(valuesLen) && (values[cut].others = true);
            ...
            for (i = 0; i < valuesLen; i++) {
                ...
                if (opts.init) {
                    var ipath = sector(cx, cy, 1000, angle, angle - 360 * values[i] / total).join(",");
                }

                var path = sector(cx, cy, r, angle, angle -= 360 * values[i] / total);
				var p = paper.path(opts.init ? ipath : path).attr(
					{ 
						fill: values[i].color || chartinst.colors[j] || "#666",
		...
		var itemCount = Math.min(valuesLen, valuesLen, cut);
		...
		chart.animate = function(mfid, f) {
			var slice = chart.slices[mfid];
			slice && f(slice);
		}
        ...
        var legend = function (otherslabel, mark, dir) {
            var x = cx + r + r / 5,
                y = cy,
                h = y + 10;
            ...
            for (var i = 0; i < itemCount; i++) {
                ...
                values[i] && values[i].others && (labels[i] = otherslabel || "Others");
                
                // labels[j] = chartinst.labelise(labels[j], values[i].legend, total);
                
                chart.labels.push(paper.set());
                chart.labels[i].push(paper[mark](x + 5, h, 5).attr({ fill: clr, stroke: "none" }));
                
                chart.labels[i].push(txt = paper.text(x + 20, h, labels[i] || values[i] && values[i].legend).attr(chartinst.txtattr).attr({ fill: opts.legendcolor || "#000", "text-anchor": "start"}));
                covers[i] && (covers[i].label = chart.labels[i]);
        ...
        if (opts.legend && valuesLen > 0) {
            legend(opts.legendothers, opts.legendmark, opts.legendpos);
        }
